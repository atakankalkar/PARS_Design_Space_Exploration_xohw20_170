...
int main()
{
	...
	if (res==0)
		...
		printf("OK!\n");
	XTime_GetTime(&tEnd);
	printf("knuth_yao_smaller_tables2 took %llu clock cycles.\n", 2*(tEnd - tStart));
	printf("knuth_yao_smaller_tables2 took %.2f us.\n",1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
	...
	XTime_GetTime(&tEnd);
	printf("knuth_yao2 took %llu clock cycles.\n", 2*(tEnd - tStart));
	printf("knuth_yao2 took %.2f us.\n",1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
	int sample_in_table;
	...
	XTime_GetTime(&tEnd);
	printf("knuth_yao_smaller_tables_single_number took %llu clock cycles.\n", 2*(tEnd - tStart));
	printf("knuth_yao_smaller_tables_single_number took %.2f us.\n",1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
	...
	if (res==0)
		...
		printf("OK!\n");
	XTime_GetTime(&tEnd);
	printf("fwd/inv_ntt2 took %llu clock cycles.\n", 2*(tEnd - tStart));
	printf("fwd/inv_ntt2 took %.2f us.\n",1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
	res = 0;
	...
	XTime_GetTime(&tEnd);
	printf("Enc/Dec took %llu clock cycles.\n", 2*(tEnd - tStart));
	printf("Enc/Dec took %.2f us.\n",1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000));
