...
void RLWE_enc2(uint16_t a[M], uint16_t c1[M], uint16_t c2[M], uint16_t m[M], uint16_t p[M])
{
	...
	unsigned int * inv_ntt2_core = (unsigned int *)0x43C00000;
	for(int ii=0;ii<128;ii++){
			   inv_ntt2_core[128+ii] = ((a[ii*2+1])<<16) | a[ii*2];
			}
		  for(int ii=0;ii<128;ii++){
		  		   inv_ntt2_core[256+ii] = ((c1[ii*2+1])<<16) | c1[ii*2];
		  		}
		  for(int ii=0;ii<128;ii++){
		  	  		   inv_ntt2_core[384+ii] = ((c2[ii*2+1])<<16) | c2[ii*2];
		  	  		}
		  for(int ii=0;ii<128;ii++){
		  	  	  		   inv_ntt2_core[512+ii] = ((m[ii*2+1])<<16) | m[ii*2];
		  	  	  		}
		  for(int ii=0;ii<128;ii++){
		  	  	  	  		   inv_ntt2_core[640+ii] = ((p[ii*2+1])<<16) | p[ii*2];
		  	  	  	  		}
			// start the core
			inv_ntt2_core[0] |=0x1;

			// checks whether it finishes or not
			while((inv_ntt2_core[0] & 0x2) !=  0x2);

			// I am sure IP core completed its execution
				for(int ii=0;ii<128;ii++){
				   c1[ii*2] =(uint16_t)inv_ntt2_core[256+ii];
				   c1[ii*2+1]=(uint16_t)(inv_ntt2_core[256+ii]>>16);
				}

				for(int ii=0;ii<128;ii++){
					c2[ii*2] =(uint16_t)inv_ntt2_core[384+ii];
					c2[ii*2+1]=(uint16_t)(inv_ntt2_core[384+ii]>>16);
						}
...
/*
	unsigned int * inv_ntt2_core = (unsigned int *)0x43C00000;
	for(int ii=0;ii<128;ii++){
			   inv_ntt2_core[128+ii] = ((c1[ii*2+1])<<16) | c1[ii*2];
			}
		  for(int ii=0;ii<128;ii++){
		  		   inv_ntt2_core[256+ii] = ((c2[ii*2+1])<<16) | c2[ii*2];
		  		}
		  for(int ii=0;ii<128;ii++){
		  	  		   inv_ntt2_core[384+ii] = ((r2[ii*2+1])<<16) | r2[ii*2];
		  	  		}
			...
			// I am sure IP core completed its execution
				for(int ii=0;ii<128;ii++){
				   c1[ii*2] =(uint16_t)inv_ntt2_core[128+ii];
				   c1[ii*2+1]=(uint16_t)(inv_ntt2_core[128+ii]>>16);
				}
