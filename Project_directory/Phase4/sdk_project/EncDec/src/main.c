...
int main()
{
	...
	for(i=0; i<10000; i++)
	{
		...
		unsigned int * encdec_core = (unsigned int *)0x43C00000;
				  ...
				  for(int ii=0;ii<128;ii++){
					  encdec_core[256+ii] = ((large_c1[ii*2+1])<<16) | large_c1[ii*2];
				  		}
				  for(int ii=0;ii<128;ii++){
					  encdec_core[384+ii] = ((large_c2[ii*2+1])<<16) | large_c2[ii*2];
				  	  		}
				  for(int ii=0;ii<128;ii++){
				  ...
				  for(int ii=0;ii<128;ii++){
					...
					// I am sure IP core completed its execution
						...
						for(int ii=0;ii<128;ii++){
							large_c1[ii*2] =(uint16_t)encdec_core[256+ii];
							large_c1[ii*2+1]=(uint16_t)(encdec_core[256+ii]>>16);
						}
